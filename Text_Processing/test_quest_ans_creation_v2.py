...
def is_multi_question_combined(question):
    ...
    multi_question_pattern = r"(\d+\.\s|[-•]\s)"
...
if __name__ == "__main__":
    ...
    ###########################################################################
    elif(choice=='refining_qa'):
        print('Refining step..')
        final_path = './chunk_checkpoint/'
        df_qa_row = pd.read_csv(final_path + 'questions_and_answers_final.csv')
        # Define the checkpoint file
        checkpoint_file = final_path + "checkpoint_refine.pkl"
        # Load checkpoint if it exists
        if os.path.exists(checkpoint_file):
            with open(checkpoint_file, "rb") as f:
                checkpoint = pickle.load(f)
                refined_answers = list(checkpoint['Answer_refined'])
                original_answers =  list(checkpoint['Answer'])
                questions = list(checkpoint['Question'])
                start_index =   checkpoint['start_index'][-1]
                done = list(checkpoint['start_index'])
            print(f"Loaded checkpoint from index {start_index}")
        else:
            refined_answers = []
            original_answers = [] 
            questions = []
        ...
        # Loop through the DataFrame starting from the checkpoint
        for index, row in df_qa_row.iterrows():
            if index < start_index - 1:  # Adjust for 0-based index
            ...
            # Save a checkpoint after every 5 entries
            if (index + 1) % 50 == 0 or (index + 1) == total_count:
                checkpoint = {
                    'Question': questions ,
                    'Answer': original_answers,
                    'Answer_refined': refined_answers,
                    'start_index': done
                }
        ...
        df_qa_refined = pd.DataFrame({
                 'Question': questions,
                 'Answer': original_answers,
                 'Answer_refined': refined_answers,
                 'start_index': done
        })
        df_qa_refined.to_csv(final_path+'questions_and_answers_refined_final.csv', index=False)
        ...
        #     print("Checkpoint file removed after successful processing.")
