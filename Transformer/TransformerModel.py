...
def preprocess_text(text):
    ...
    # Keep alphanumeric characters and common punctuation (like . , ! ? ' )
    text = re.sub(r'[^A-Za-z0-9.,!?\' ]+', '', text)
...
def load_and_process_data(path_aq):
    data_original = pd.read_csv(path_aq)
    data_original=data_original[['Question','Answer_refined']]
    data_original = data_original.rename(columns={'Answer_refined': 'Answer'})
    ...
    spm.SentencePieceTrainer.train(input=temp_file_ans, model_prefix='tokenizer_a', vocab_size=9378, model_type='unigram')
    ...
    spm.SentencePieceTrainer.train(input=temp_file_ques, model_prefix='tokenizer_q', vocab_size=6370, model_type='unigram')
...
def restore_punctuation(text, tk_T5,model_T5 ):
    # Tokenize the input text
    inputs = tk_T5(text, return_tensors="tf")

    # Generate the restored text
    outputs = model_T5.generate(**inputs, max_length=512)

    # Decode the output tokens to text
    restored_text = tk_T5.decode(outputs[0], skip_special_tokens=True)
...
if __name__ == "__main__":
    ...
    if (choice== 'train'):
        ...
        # Load the tokenizer and the BERT model for punctuation restoration
        # tk_P =  T5Tokenizer.from_pretrained('SJ-Ray/Re-Punctuate')
        # model_P = TFT5ForConditionalGeneration.from_pretrained('SJ-Ray/Re-Punctuate')
        ...
        for i in random_indices: #range(20,25):
            ...
            ## add punctuations: this is a test using HuggingFace lib #########
            # inputs = tk_P.encode("punctuate: " + pred_string, return_tensors="tf") 
            # result_pred = model_P.generate(inputs)
            # decoded_output_pred = tk_P.decode(result_pred[0], skip_special_tokens=True)
            
            # result_test = model_P.generate(test_a.values[i])
            # decoded_output_test = tk_P.decode(result_test[0], skip_special_tokens=True)
            # 
        ...
        ### custom questions ###################################################
        qa_vocabulary = {
            "What is the name of Harry Potter's school?": "Hogwarts School of Witchcraft and Wizardry",
            ...
            "Who teaches Potions at Hogwarts?": "Severus Snape",
